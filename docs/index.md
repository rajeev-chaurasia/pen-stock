...
Every published number comes from one of these, and each is runnable
from a clean checkout.

| Tool | What it produces |
|---|---|
| [bench/README.md](../bench/README.md) | The benchmark harness and its methodology, including why gateway cost is only ever reported as a difference. Read it before quoting any latency figure here. |
| [cmd/cachestudy](../cmd/cachestudy) | The 257 probe threshold sweep behind [cache-quality.md](cache-quality.md) and the 43.9 percent false hit rate. |
| [cmd/calibrate](../cmd/calibrate) | The latency profiles in `bench/profiles/`, recorded from real provider traffic so the simulator replays something that happened. |
| [cmd/llmsim](../cmd/llmsim) | The deterministic mock provider every benchmark arm runs against, so a measurement is of the gateway rather than of somebody's API that day. |
| [cmd/plot](../cmd/plot) | The SVG figures in `docs/img/`, regenerated byte for byte from the committed result JSON. |
